...
public class SecretQuestionCredentialProvider implements CredentialProvider<CredentialModel>, CredentialInputValidator/*, OnUserCache*/ {
    ...
    public boolean isValid(RealmModel realm, UserModel user, CredentialInput credentialInput) {
        ...
        String challengeResponse = userInput.getChallengeResponse();
        if (challengeResponse == null || ObjectUtil.isBlank(userInput.getCredentialId())) {
            logger.error("Invalid credential input for user: " + user.getUsername());
            return false;
        }

        List<CredentialModel> storedCreds = user.credentialManager()
        ...
        try {
            ...
            PBEKeySpec spec = new PBEKeySpec(challengeResponse.toCharArray(), salt, model.getSecretQuestionCredentialData().getHashIterations(), 256);
