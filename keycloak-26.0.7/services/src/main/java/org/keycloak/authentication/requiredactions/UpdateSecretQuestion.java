...
public class UpdateSecretQuestion implements RequiredActionProvider, RequiredActionFactory, CredentialRegistrator {
    ...
    public void processAction(RequiredActionContext context) {
        ...
        String challengeResponse = formData.getFirst("secretQuestion");
        String totpSecret = formData.getFirst("secretQuestionSecret");
        String mode = formData.getFirst("mode");
        String userLabel = formData.getFirst("userLabel");

        SecretQuestionCredentialModel credentialModel = SecretQuestionCredentialModel.createFromPolicy(context.getRealm(), totpSecret, userLabel);
        ...
        if (!CredentialHelper.createSecretQuestionCredential(context.getSession(), context.getRealm(), context.getUser(), challengeResponse, credentialModel)) {
            Response challenge = context.form()
                    .setAttribute("mode", mode)
