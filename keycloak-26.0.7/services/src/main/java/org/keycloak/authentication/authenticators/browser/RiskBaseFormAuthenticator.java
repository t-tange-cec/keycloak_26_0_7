...
public class RiskBaseFormAuthenticator extends AbstractUsernameFormAuthenticator implements Authenticator, CredentialValidator<SecretQuestionCredentialProvider> {
    ...
    public void action(AuthenticationFlowContext context) {
        validateSecretQuestion(context);
    }
    ...
    public void validateSecretQuestion(AuthenticationFlowContext context) {
        MultivaluedMap<String, String> inputData = context.getHttpRequest().getDecodedFormParameters();

        String secretQuestion = inputData.getFirst("secretQuestion");

        String credentialId = inputData.getFirst("selectedCredentialId");

        if (credentialId == null || credentialId.isEmpty()) {
            SecretQuestionCredentialModel defaultSecretQuestionCredential = getCredentialProvider(context.getSession())
                    .getDefaultCredential(context.getSession(), context.getRealm(), context.getUser());
            credentialId = defaultSecretQuestionCredential==null ? "" : defaultSecretQuestionCredential.getId();
        }
    ...
    protected String disabledByBruteForceFieldError() {
        return Validation.FIELD_SECRET_QUESTION;
    }

    @Override
    protected Response createLoginForm(LoginFormsProvider form) {
        return form.createLoginSecretQuestion();
    }
